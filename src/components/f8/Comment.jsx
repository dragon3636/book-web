    ...
    <div>
      <ul>
        {leason.map((item, index) => (
          <li
            className={`font-normal text-xl  hover:text-red-300 active:text-red-500 cursor-pointer ${
              topic === index + 1 ? 'text-red-500' : 'text-red-100'
            }`}
            value={index + 1}
            onClick={handleLeason}
            key={index}
          >
            {item}
          </li>
